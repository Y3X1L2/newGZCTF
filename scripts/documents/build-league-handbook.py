"""Generate the phase-one team handbook from its Markdown source."""
...
SOURCES = ["phase1-team-handbook.md"]
OUTPUT = DIRECTORY / "联赛第一阶段开发分工手册V1.0.docx"
    ...
    for part in re.split(r"(`[^`]+`|\[[^\]]+\]\([^)]+\))", text):
        ...
        if match:
            ...
            if not target.startswith(("https://", "http://")):
                ...
                target = "https://github.com/Y3X1L2/newGZCTF/blob/codex/league-handbook-20260921/" + relative
...
def add_table(doc, lines, source):
    ...
    if len(rows[0]) == 2:
        ...
        widths = {
            ...
            "步骤": [1.8, 10.0, 4.6], "能力提供者": [4.0, 4.3, 8.1],
            "页面任务": [3.4, 4.4, 8.6],
...
def build(sources=SOURCES, output=OUTPUT,
          title="联赛第一阶段开发分工手册",
          subject="按比赛流程分工、接口交接、并行开发和联合验收",
          date="2026-09-21"):
    doc = Document()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--title", default="联赛第一阶段开发分工手册")
    parser.add_argument("--subject", default="按比赛流程分工、接口交接、并行开发和联合验收")
