"""Generate the dual-port competition background and flow document."""
...
SOURCES = ["dual-port-competition-flow.md"]
OUTPUT = DIRECTORY / "双港调度争夺比赛背景与总流程V1.0.docx"
    ...
    for part in re.split(r"(`[^`]+`|\[[^\]]+\]\([^)]+\))", text):
        ...
        if match:
            ...
            if not target.startswith(("https://", "http://")):
                ...
                target = "https://github.com/Y3X1L2/newGZCTF/blob/codex/league-story-20260921/" + relative
...
def build(sources=SOURCES, output=OUTPUT,
          title="双港调度争夺比赛背景与总流程",
          subject="故事背景、比赛阶段、胜负结局和后续玩法方向",
          date="2026-09-21"):
    doc = Document()
    ...
    for name, face, size in (("Roadmap Table", "SimSun", 11), ("Roadmap Table Header", "SimHei", 11),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--title", default="双港调度争夺比赛背景与总流程")
    parser.add_argument("--subject", default="故事背景、比赛阶段、胜负结局和后续玩法方向")
