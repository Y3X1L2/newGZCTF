...
先用 [`storage-health-report.sql`](../../../scripts/database/sql/storage-health-report.sql) 在受控只读连接采集表/索引/分区大小及统计估计，按逻辑父表汇总叶分区；输出保存到仓库外。脚本不含业务正文和查询文本。执行方式、字段解释、统计重置及 WAL/对象存储的边界见[存储与演进审查](../database-storage-and-evolution-review.md#43-如何判断实际是否太大)。不要仅凭备份文件大小、dead tuple 估计或 `idx_scan=0` 决定扩容、重写表或删索引。

除保留天数外还需比较过期数据到达速率与清理速率：默认单表每小时一批 1,000 行，运行失败还会减少实际吞吐。确认 `teamlab-flow` 清理成功后，继续核验本周期后续终态/指标清理；单次分区成功不能替代整个周期成功。
