...
- [x] 清理隔离数据库恢复资源，完成本地交接；远端临时文件和 GitHub 文档同步按网络限制留待后续处理。
...
生产发布完成；用户确认业务测试完成并删除临时题。本地收尾沿用禁止网络连接的要求，没有重新检查删除后的数据库、容器、端口或队列，不把用户反馈写成自动化销毁证据。
...
5. 远端三个临时上传文件 `/tmp/gzctf-pr9-rollout.py`、`/tmp/gzctf-pr9-post-probe.py`、`/tmp/gzctf-pr9-rollout-20260908.tar.gz` 未在禁止网络连接后清理或复核；用户尚未反馈其清理结果。正式 release、root 备份、`.31` 已安装的恢复脚本/drop-in 及本地候选归档必须保留。
6. 本轮只修改交接与状态文档，按文档范围做差异和链接检查，不重跑候选已通过的全量构建/测试。未 fetch 或推送；远端最后核验 main 为 `3a701269...`，运行代码与发布候选相同，GitHub 尚不包含本轮收尾文档。网络限制解除后，先 fetch 核对远端，再正常推送任务分支和合并主线，禁止 force push。
7. 本地进程检查未发现仍在运行的本任务 `ssh_session.py` 辅助进程；未再次打开浏览器或建立 SSH 连接。任务 worktree 暂留以交付未推送文档、忽略目录内的发布包和重放工具，其他 worktree 和 PVE 独立改动未修改。
