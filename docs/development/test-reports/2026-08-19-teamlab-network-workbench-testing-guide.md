...
**测试点**：拖入 5 种节点各 1 个；交换机↔设备连线、交换机↔交换机路由连线；改网段 CIDR；折叠/展开区域；自动排版；保存后再打开内容还在。

### 2.3 校验与发布（Validate / Publish）

- **校验**：点上面的「校验」按钮，右侧会出现**校验抽屉**，列出所有"不能部署"的问题（比如：设备没接任何网段、网段没有出口、网段 CIDR 冲突、镜像没指定……）。问题分错误和警告。**有阻断项时不能发布**。
