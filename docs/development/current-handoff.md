...
> 2026-09-08 更新：稳定标签 `stable-20260908` 已推送，精确指向生产发布 `ab2bd54b`；本地/GitHub 主线与发布提交仅文档不同。用户确认测试完成并删除临时题，解除网络限制后已只读复核题/实例/容器记录不存在、活动运行任务为 0，未打开靶机。当前接手入口为 [稳定基线说明](handoffs/2026-09-08-stable-baseline.md)，备份和发布证据见 [生产发布交接](handoffs/2026-09-08-pr9-production-rollout.md)。已知 health 降级、保留任务 SQL 错误和混合 Agent 版本仍须按说明管理；下方 2026-09-01 的 release、路径和端口仅作历史背景。
