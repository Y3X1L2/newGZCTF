...
> 2026-09-08 更新：`.27` 主站及本机 Agent 已发布 `ab2bd54b`，预备和最终备份均通过实际恢复验证；旧 `9eef8ac` 保留为回退目标。自动化完成临时练习创建和容器入口访问，用户接手后确认测试完成并删除题目。后续仅按追加授权同步 GitHub 文档、清理三个指定临时上传文件并关闭 SSH，没有恢复业务或靶机测试，也未重新核验容器销毁。已知 health 历史迁移降级、数据保留 SQL 错误及一次已恢复的指标并发冲突详见 [生产发布交接](handoffs/2026-09-08-pr9-production-rollout.md)。下方 2026-09-01 的 release、路径和旧端口说明仅作历史背景，当前事实以 `current-state.md` 为准。
