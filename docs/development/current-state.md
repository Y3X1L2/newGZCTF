...
## 主线源码与 CI 核查（2026-09-19）

本次核对远端 main 为 `f389d26c548802d6caabd974efd3053d4dcbd0bd`，包含下述 TeamLab P1/P2/P3 相关改动。历史“候选分支”“未合并”描述按其记录时点理解，不再用于判断这些代码是否进入 main；生产和测试服务器发布状态仍以现场记录为准，本次未连接服务器。

该提交的 [Quality 34990764269](https://github.com/Y3X1L2/newGZCTF/actions/runs/34990764269) 前端通过，后端单元测试 1145/1145 通过，集成测试 294 通过、5 失败：两项 OpenAPI 快照不一致、一项跨 Token 会话操作超时、两项镜像删除接口返回 500。后续迁移、查询计划和 OpenAPI 兼容步骤未继续执行。并行开发先修复并复验这一基线，不能把定向报告中的通过结果替代完整 CI。

GitHub 查询显示 main 尚未配置分支保护，仓库 ruleset 列表为空；相关 PR 检查与审查设置列入联赛开发准备事项。详细源码、运行报告和协作方案见 [联赛路线评估](league/roadmap-20260919.md)。

## TeamLab API 底座测试记录（2026-09-15）

TeamLab API 改动来自分支 `codex/teamlab-api-product-integration`，P3 性能优化基线为 `d944a10`，双 Worker 验证提交为 `f389d26c`，已进入当前 main。P1 已用正式发布包、真实 Agent 和真实 OVN/OVS 跑通完整 API 流程；P2 已完成 Open API、默认布局、画布配色、状态表达、会话审计、服务开放、文件运维、连接器登记等产品与调用闭环，并删除无价值的启动依赖和端点采集链。
