...
当前接手入口为 [2026-09-08 生产发布交接](handoffs/2026-09-08-pr9-production-rollout.md)：`.27` 已发布 `ab2bd54b`，两份备份恢复和后置检查通过，用户确认测试完成并删除临时题。运行代码与核验的本地/GitHub 主线一致，之后的 Git SHA 差异来自文档；追加授权仅用于 GitHub 文档同步和指定临时文件清理，已核对并清理三个远端临时文件。优先处理独立的数据保留 SQL 缺陷、指标并发回归和现有专项缺口；不得将本次有限联网授权扩展为恢复业务或靶机测试。
