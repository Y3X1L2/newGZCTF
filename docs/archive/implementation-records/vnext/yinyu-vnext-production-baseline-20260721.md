...
- Windows VM 模型和 Guacamole 连接接口存在共享默认 RDP 凭据，当前镜像链路没有实例级密码注入契约。
- Guacamole 在未配置认证 Token 时会回退到默认管理员凭据，不符合生产密钥管理要求。

这些问题分别进入后续安全加固、发布物和部署演练阶段处理；在候选版本通过全部门禁前，不直接调整现网服务权限和运行方式。
