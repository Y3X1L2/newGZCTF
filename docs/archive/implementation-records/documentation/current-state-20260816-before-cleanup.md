...
| 稳定分支 | `main` |
| 本快照代码基线 | `55db938` |
| 基线说明 | Phase 9 TeamLab 基础与自主练习模块；练习模块已合并主线并完成本地启动、迁移和核心业务验收，尚未部署生产 |
| 本机正式工作区 | `D:\Work\newGZCTF` |
...
- PR #5 已合并到 `main`，提供 `/practice`、题库浏览、练习详情、统计、后台题库管理、Flag/附件和 Docker 实例流程；运行实例继续复用 `DeploymentQueueTicket`，没有建立第二套队列。
- 主线修复提交 `8de8570` 解决测试构造器编译阻断、禁用草稿无法经 Open API 管理、练习列表游标分页缺失，并增加真实 HTTP 核心流程和历史迁移兼容回归。
...
- 当前代码已进入稳定主线，但仅完成本地验收，未部署到 `10.24.0.27`；生产发布状态仍以本文第 5 节为准。
...
- 自主练习：公共练习题库、来源导入、筛选、附件、多 Flag、Docker 实例、提交、统计和后台管理。
...
截至本快照，首页、赛事、战队、培训、认证、个人主页、理论、自主练习、TeamLab 及主要通用管理页面已经接入 vNext。
...
| Phase 8、10-14 | 尚未按总纲全部完成 | VM 统一、内容资产/出题 API、培训理论增强、AWDP 展示和商业验收；自主练习代码已先行进入主线，仍待生产验收 |
...
- 过期培训实例恢复和运行引用安全重载。
- 培训理论提交后的答案回顾。
- Windows VM 使用镜像内固定 RDP 账号，支持浏览器远程桌面与比赛场景原生 `.rdp` 入口；当前仍缺少完成双实例真实验收的认证镜像。
- 首页编排动画和丝带绘制时序优化。
