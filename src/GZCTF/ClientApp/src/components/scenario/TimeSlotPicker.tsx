...
import { Card, Button, Badge, Group, Text, Alert } from '@mantine/core';
...
export default function TimeSlotPicker({ scenarioId, onReserved }: TimeSlotPickerProps) {
  ...
  useEffect(() => {
    fetch(`/api/v1/scenarios/${scenarioId}/timeslots`)
      .then(r => r.json())
      .then(data => { setSlots(data.items ?? data); setLoading(false); })
  ...
  return (
    <div data-testid="timeslot-picker">
      <Text fw={500} mb="md">选择参与时间段</Text>
      {slots.map(slot => {
        const isFull = slot.currentParticipants >= slot.maxParticipants;
        return (
          <Card key={slot.id} shadow="sm" padding="md" mt="sm" withBorder
