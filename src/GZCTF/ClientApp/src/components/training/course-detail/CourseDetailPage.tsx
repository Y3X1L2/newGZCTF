...
import { trainingCourseProgress } from '@Components/training/TrainingCourseUI'
...
import {
  ...
  emptyChallengeDraft,
  emptyCourseDraft,
  emptyDockerRegisterDraft,
  emptyLocalImportDraft,
  emptyResourceDraft,
...
import { AddStudentModal, AddTeacherModal } from './CoursePeopleDialogs'
...
const CourseDetail: FC = () => {
  ...
  const publishCourse = async () => {
    if (!course) return
    try {
      await trainingCourseAdminApi.publish(course.id)
      await load()
    } catch (e) {
      showErrorMsg(e, t)
    }
  ...
  if (!course) {
    return (
      <WithNavBar isLoading width="min(112rem, calc(100% - (var(--yy-page-gutter) * 2)))">
        <></>
      </WithNavBar>
    )
  }
  ...
  return (
    <WithNavBar width="var(--container)" minWidth={0}>
      ...
      <StudentLearningDrawer
        ...
        loading={studentLearningLoading}
      ...
      <AddStudentModal
        ...
        saving={saving}
        onClose={() => setStudentOpened(false)}
        onKeywordChange={setStudentKeyword}
        onSearch={() => void searchStudentCandidates()}
        onSelect={setSelectedStudentId}
        onAdd={() => void addCourseStudent()}
      />
      ...
      <AddTeacherModal
        ...
        selectedRole={selectedTeacherRole}
        saving={saving}
        onClose={() => setTeacherOpened(false)}
        onKeywordChange={setTeacherKeyword}
        onSearch={() => void searchTeacherCandidates()}
        onSelect={setSelectedTeacherId}
        onRoleChange={setSelectedTeacherRole}
        onAdd={() => void addCourseTeacher()}
      />
