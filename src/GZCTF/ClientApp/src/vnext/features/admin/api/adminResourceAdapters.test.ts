...
describe('imageTemplateAdminApi', () => {
  ...
  it('maps deployed numeric remote access enums to the feature contract', async () => {
    ...
    await expect(adapter.remoteAccess(1)).resolves.toEqual({
      ...
      port: 22,
      username: null,
      credentialMode: 'platformGenerated',
      hasCredential: false,
      updatedAt: null,
    })
  })

  it('serializes the feature remote access contract to deployed numeric enums', async () => {
    ...
    })).resolves.toMatchObject({ protocol: 'rdp', credentialMode: 'existingAccount' })
