...
export function RemoteSessionsPanel({ runtimeId }: { runtimeId?: string }) {
  ...
  const sessions = useSWR(['teamlab:remote-sessions', runtimeId, status, protocol, abnormalOnly, search, after],
    () => teamLabRemoteAccessApi.list({ runtimeId, status: status ? Number(status) : undefined,
      protocol: protocol ? protocol as keyof typeof protocolLabels : undefined, abnormalOnly, query: search || undefined, after }),
    { refreshInterval: (latest) => latest?.items.some(item => !['ended', 'failed'].includes(item.session.status)) ? 5000 : 0 })
  const end = async () => {
  ...
  return <section className={styles.panel}>
    ...
    <form className={`${styles.captureForm} ${styles.remoteSessionFilters}`} onSubmit={(event) => { event.preventDefault(); setAfter(undefined); setSearch(query.trim()) }}>
      <label><span>关键词</span><input placeholder="用户名、资产名、节点名或会话号" value={query} onChange={(event) => setQuery(event.currentTarget.value)} /></label>
      ...
      <label><span>连接方式</span><select value={protocol} onChange={(event) => { setProtocol(event.currentTarget.value); setAfter(undefined) }}><option value="">全部方式</option>{Object.entries(protocolLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
      <label><span>仅看异常</span><input checked={abnormalOnly} onChange={(event) => { setAbnormalOnly(event.currentTarget.checked); setAfter(undefined) }} type="checkbox" /></label>
      <ActionButton icon={<Search size={16} />} type="submit">查询</ActionButton>
    </form>
    {sessions.error ? <InlineFeedback tone="danger">{errorMessage(sessions.error, '会话列表读取失败。')}</InlineFeedback> : sessions.isLoading ? <DataState loading title="正在读取会话" /> : !sessions.data?.items.length ? <DataState title="没有符合条件的会话" /> : <div className={styles.sessionTableScroll}><table className={styles.sessionTable}><thead><tr><th>资产</th><th>状态 / 连接方式</th><th>操作者 / 节点</th><th>原因 / 到期时间</th><th>操作</th></tr></thead><tbody>{sessions.data.items.map(({ session, workerNodeId, workerNodeName, requestedByUserId, requestedByName }) => <tr key={session.id} title={`会话 ${session.id} · 用户 ${requestedByUserId} · 节点 ${workerNodeId}`}>
      <td>{session.assetName}</td>
      <td>{session.endReason === 'service_restarted' ? '连接中断' : statusLabels[session.status]}<small>{protocolLabels[session.protocol]}</small>{session.endReason && session.endReason !== 'service_restarted' ? <small>{session.endReason}</small> : null}</td>
      <td>{requestedByName}<small>{workerNodeName}</small></td>
