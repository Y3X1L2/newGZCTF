...
const runtime: TeamLabRuntime = {
  ...
  assets: [{ key: 'web', name: 'Web 服务', kind: 'docker', runtimeResourceId: 'container-1', primaryIp: '10.10.0.10', status: 'running', error: null }],
...
describe('TeamLabRuntimeDetailPage', () => {
  beforeEach(() => {
    vi.mocked(useTeamLabRuntime).mockReturnValue({ runtime, error: undefined, isLoading: false, isRefreshing: false, mutate: vi.fn() })
    vi.mocked(useRuntimeEvents).mockReturnValue({ events: [], error: undefined, isLoading: false, isRefreshing: false, mutate: vi.fn() })
    vi.mocked(useTrafficObservability).mockReturnValue({
      flows: { page: { items: [], nextCursor: null }, error: undefined, isLoading: false, isRefreshing: false, cursor: { cursor: null, page: 1, canGoBack: false, next: vi.fn(), previous: vi.fn(), reset: vi.fn() }, mutate: vi.fn() },
      paths: { page: { items: [], nextCursor: null }, error: undefined, isLoading: false, isRefreshing: false, cursor: { cursor: null, page: 1, canGoBack: false, next: vi.fn(), previous: vi.fn(), reset: vi.fn() }, mutate: vi.fn() },
    })
  })

  it('renders persisted stage, shard placement and topology without unsupported controls', () => {
    render(<MemoryRouter initialEntries={[`/admin/teamlab/topology-a/runtimes/${runtime.id}`]}><Routes><Route path="/admin/teamlab/:topologyId/runtimes/:runtimeId" element={<TeamLabRuntimeDetailPage />} /></Routes></MemoryRouter>)
