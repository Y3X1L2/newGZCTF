...
export function TeamLabWorkspacePage() {
  ...
  if (request.isLoading) return <div className={styles.state}><DataState description="正在读取队伍环境和任务目标。" loading title="渗透工作区加载中" /></div>
  if (request.error || !request.workspace)
    return <div className={styles.state}><DataState description={errorMessage(request.error, '队伍环境尚未部署或当前不可访问。')} title="无法打开渗透工作区" /></div>
  ...
  return (
    <div className={styles.page}>
      <header className={styles.pageHeader}>
        <div><span>PENETRATION WORKSPACE</span><h1>渗透演练</h1></div>
        <ActionButton
      ...
      {workspace.status === 'failed' ? <InlineFeedback tone="danger">环境部署失败，请联系比赛管理员查看运行日志。</InlineFeedback> : null}
      ...
      <div className={styles.contentGrid}>
        ...
        <PlayerAccessPanel gameId={gameId} ready={workspace.status === 'running'} />
      </div>
      <PlayerResetDialog
        gameId={gameId}
