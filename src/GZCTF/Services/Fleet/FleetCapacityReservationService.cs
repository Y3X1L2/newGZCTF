...
public sealed class FleetCapacityReservationService
{
        ...
        CancellationToken token)
    {
        ...
        var evaluated = (await snapshots.LoadAsync(token))
            ...
            .Select(item => new CapacityCandidate(
                ...
                request.RequireRemote && item.Node.IsLocal
                    ...
                    : eligibility.GetReason(item, request.RequiredCapability, requested,
                        request.RequireTeamLab, request.RequiredFeatures)))
            .ToArray();
        var candidates = evaluated
            .Where(item => item.RejectionReason is null)
            .OrderByDescending(item => eligibility.Score(item.Snapshot, requested))
        ...
        logger.LogInformation("Reserved capacity for ticket {TicketId} on node {NodeId}: Docker={Docker}, VM={Vm}",
            ticketId, selected.Snapshot.Node.Id, requested.DockerSlots, requested.VmSlots);
        return FleetCapacityReservationResult.Reserved(selected.Snapshot.Node, requested);
    }
    ...
    internal static string BuildUnavailableMessage(WorkloadResourceVector resources,
        IEnumerable<string> rejectionReasons)
    {
        ...
        return $"No eligible node for {Format(resources)}. {detail}";
    }
