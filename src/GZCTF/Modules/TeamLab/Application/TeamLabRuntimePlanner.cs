        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var asset in definition.Assets.OrderBy(item => item.Key, StringComparer.Ordinal))
        {
            ...
            runtime.Assets.Add(new TeamLabRuntimeAsset
            {
                ...
                ImageDigest = asset.ImageDigest ?? templateDigests[resolvedTemplateIds[asset.Key]].ImageHash,
                Image = asset.Kind == TeamLabAssetKind.Docker
                    ? DockerImageReference.ResolvePullTarget(
                        templateDigests[resolvedTemplateIds[asset.Key]].Name,
                        templateDigests[resolvedTemplateIds[asset.Key]].RegistryUrl).FullImage
                    : null
            });
        }
