...
public static class TeamLabResourceNameFactory
{
    ...
    public static string ServiceGatewayInterface(int runtimeId, string networkKey) =>
        LinuxName($"tlsg{runtimeId}-{networkKey}");
    ...
    public static string ServiceGatewayMac(Guid runtimePublicId, int generation, string networkKey) =>
        $"02:{Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(
            $"{runtimePublicId:D}:{generation}:service-gateway:{networkKey}")))[..10].ToLowerInvariant().Insert(2, ":").Insert(5, ":").Insert(8, ":").Insert(11, ":")}";
