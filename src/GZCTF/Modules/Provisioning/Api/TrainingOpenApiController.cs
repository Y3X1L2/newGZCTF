...
[Route("api/open/v1/training/courses")]
[Produces(MediaTypeNames.Application.Json, "application/problem+json")]
public sealed class TrainingOpenApiController(
    AcademicImportApplicationService imports,
    IAuthorizationService authorization) : ControllerBase
{
    [HttpPost("import")]
    [Authorize(Policy = "scope:" + ApiTokenScopes.TrainingWrite)]
    [ProducesResponseType(typeof(ApiOperationModel), StatusCodes.Status202Accepted)]
    public async Task<IActionResult> Import(
        [FromBody] TrainingCourseImportBatchModel model,
        [FromHeader(Name = "Idempotency-Key"), Required] string idempotencyKey,
        CancellationToken cancellationToken)
    {
        await RequireResourceAsync("training-course", "*");
        var (tokenId, actorUserId) = GetActor();
        var result = await imports.SubmitTrainingCoursesAsync(
